...
| Authoritative ref value and compare-and-swap | Git gateway / repository service |
| Ref query projection and ref audit events | D1, written by and reconciled with the Git gateway |
...
7. A Git gateway compares its authoritative ref with the caller's expected OID, verifies quarantined objects, publishes the ref, then records a unique `repository_ref_events` event and updates the D1 projection.
...
Git object upload may occur before the D1 transaction. The Git ref CAS is the publication point. D1 is updated through an idempotent gateway event/outbox flow; a reconciliation worker compares authoritative Git refs with `repository_refs` after crashes. Product UI must expose reconciliation state rather than treating a queued projection update as a successful push.
...
`functions/api/git.ts` is now an authenticated control-plane endpoint. It reads canonical repositories/refs, registers a fork origin from server-resolved projections, and refuses smart-HTTP or ref mutations with `501` until a real gateway is configured. It no longer advertises an incomplete Git protocol or treats D1 as a Git object/ref server.

The Git gateway must eventually:

1. Authenticates the actor and authorizes the repository/ref operation.
2. Loads the authoritative Git ref and rejects a stale `expectedOldOid`.
4. Verifies the new object exists in Git quarantine.
5. Atomically publishes the Git ref in the repository service.
6. Updates the D1 `repository_refs` projection and inserts `repository_ref_events` idempotently.
