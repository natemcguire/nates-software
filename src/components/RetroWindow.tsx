...
export const RetroWindow: React.FC<RetroWindowProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-2">
          <button
            ...
            className="w-5 h-5 bg-w95-gray w95-border flex items-center justify-center text-black hover:bg-white active:translate-x-0.5 text-xs font-bold"
            title="Minimize"
          >
            <Minus size={12} />
          </button>
          ...
          <button
            ...
            className="w-5 h-5 bg-w95-gray w95-border flex items-center justify-center text-black hover:bg-red-700 hover:text-white active:translate-x-0.5 text-xs font-bold"
            title="Close"
          >
            <X size={12} />
          </button>
      ...
      {/* Body Content */}
      <div className="flex-1 overflow-hidden p-3 flex flex-col bg-[#ece9d8]">
        {children}
      </div>
      ...
      {!windowState.isMaximized && (
        <>
          <div onPointerDown={(e) => startResize('n', e)} className="absolute top-0 left-2 right-2 h-2 cursor-n-resize" />
          <div onPointerDown={(e) => startResize('s', e)} className="absolute bottom-0 left-2 right-2 h-2 cursor-s-resize" />
          <div onPointerDown={(e) => startResize('e', e)} className="absolute top-2 bottom-2 right-0 w-2 cursor-e-resize" />
          <div onPointerDown={(e) => startResize('w', e)} className="absolute top-2 bottom-2 left-0 w-2 cursor-w-resize" />
          
          <div onPointerDown={(e) => startResize('nw', e)} className="absolute top-0 left-0 w-3 h-3 cursor-nw-resize" />
          <div onPointerDown={(e) => startResize('ne', e)} className="absolute top-0 right-0 w-3 h-3 cursor-ne-resize" />
          <div onPointerDown={(e) => startResize('sw', e)} className="absolute bottom-0 left-0 w-3 h-3 cursor-sw-resize" />
          <div onPointerDown={(e) => startResize('se', e)} className="absolute bottom-0 right-0 w-4 h-4 cursor-se-resize flex items-end justify-end p-0.5">
