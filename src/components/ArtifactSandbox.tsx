          ...
          <div className="bg-[#1e293b] border-2 border-slate-600 rounded-lg max-w-lg w-full shadow-2xl p-5 text-slate-100 font-sans text-xs space-y-4">
            <div className="flex items-center justify-between border-b border-slate-700 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm text-white font-mono">Launch Headless AI Agent for {app.name}</span>
              </div>
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              We do not host models on the server. Run your own local agent on your machine inside the isolated worktree:
            </p>

            <div className="space-y-2 font-mono">
              <div className="bg-slate-950 p-2.5 rounded border border-slate-800 flex items-center justify-between">
                <code className="text-purple-300 text-xs">claude "Mod {app.name} in /tmp/slop-{app.id}"</code>
                <button
                  onClick={() => {
                    playClickSound();
                    navigator.clipboard.writeText(`claude "Mod ${app.name} in /tmp/slop-${app.id}"`);
                    showAlert("Claude Code launch command copied!", "Command Copied", "success");
                  }}
                  className="bg-purple-900 hover:bg-purple-800 text-purple-200 px-2 py-1 rounded text-[10px]"
                >
                  Copy
                </button>
              </div>
              ...
              <div className="bg-slate-950 p-2.5 rounded border border-slate-800 flex items-center justify-between">
                <code className="text-sky-300 text-xs">agy "Implement features for {app.name}"</code>
                <button
                  onClick={() => {
                    playClickSound();
                    navigator.clipboard.writeText(`agy "Implement features for ${app.name}"`);
                    showAlert("AGY launch command copied!", "Command Copied", "success");
                  }}
                  className="bg-sky-900 hover:bg-sky-800 text-sky-200 px-2 py-1 rounded text-[10px]"
                >
                  Copy
                </button>
              </div>
            ...
            <div className="flex justify-end">
              <button
                ...
                className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-1.5 rounded font-bold font-mono"
              >
                Done
              </button>
            </div>
